...
(function (window) {
	...
	Dsy.prototype.inset = function (obj, b, c, arr) {
		var t = this;
		if(arr) {
			var aNum = '';
			var bNum = '';
			...
			for(var i=0; i<this.Items['0'].length; i++) {
				...
				oPtion.value = oPtion.innerHTML = this.Items['0'][i]

				if(arr[0] == this.Items['0'][i]) {
					aNum = i;
			...
			for(var i=0; i<this.Items[('0_' + aNum)].length; i++) {
				var oPtion = document.createElement('option');
				oPtion.value = oPtion.innerHTML = this.Items[('0_' + aNum)][i]

				if(arr[1] == this.Items[('0_' + aNum)][i]) {
					bNum = i;
					t.limit = ('0_' + aNum);
					oPtion.selected = true;
				}
			...
			for(var i=0; i<this.Items[(t.limit + '_' + bNum)].length; i++) {
				var oPtion = document.createElement('option');
				oPtion.value = oPtion.innerHTML = this.Items[(t.limit + '_' + bNum)][i]

				if(arr[2] == this.Items[(t.limit + '_' + bNum)][i]) {
		...
		this.change(obj, b, c);
		t.change2(b, c, t.limit);

	}

	Dsy.prototype.change = function (obj, b, c) {
		var t = this;
		obj.onchange = function () {
			b.innerHTML = '<option value=0>请选择城市</option>';
			for(var i=0;i< t.Items['0'].length; i++) {
				if(obj.value == t.Items['0'][i]) {
					t.limit = ('0_' + i);
					for(var j=0; j<t.Items[('0_' + i)].length; j++) {
						var oPtion = document.createElement('option');
						oPtion.value = oPtion.innerHTML = t.Items[('0_' + i)][j];
						b.appendChild(oPtion);
					}
				}
			}
	...
	Dsy.prototype.change2 = function (obj, c, limit) {
		var t = this;
		obj.onchange = function () {
			c.innerHTML = '<option value=0>请选择区/县</option>';
			for(var i=0;i< t.Items[limit].length; i++) {
				if(obj.value == t.Items[limit][i]) {
					for(var j=0; j < t.Items[limit+ '_' + i].length; j++) {
						var oPtion = document.createElement('option');
						oPtion.value = oPtion.innerHTML = t.Items[limit+ '_' + i][j];
						c.appendChild(oPtion);
					}
				}
			}
